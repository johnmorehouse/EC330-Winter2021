...
_December 2, 1970_: Environmental Protection Agency  (EPA) is Established  
  ...
  - With it: The Clean Air Act expands scope and power
...
```{R, air_transport, out.width = "70%", out.height = "70%", echo = F}
include_graphics("images/coal_transit.png")
```
...
```{R, nonattain_transport, out.width = "70%", out.height = "70%", echo = F}
include_graphics("images/non-attain.png")
```
...
- Again: __in equilibrium__, .purple[utility is equal across locations]. 



- How can we use locational eq to "fix up" our last example?
...
# Another Problem
 ...
 People move and utility is equal across all locations
...
- Utility: $U(w_{j}, r_{j}(L_j), a_{j}) = w_j - .5\times r_j(L_j) + a_j$
